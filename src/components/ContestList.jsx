...
const ContestList = () => {
  ...
  useEffect(() => {
    const fetchAvailableProblems = async () => {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:3000/api/problems/getall', {
          headers: {
        ...
        if (Array.isArray(response.data)) {
          ...
          if (response.data.problems) {
            ...
            const possibleProblems = Object.values(response.data).find(val => Array.isArray(val));
            if (possibleProblems) {
        ...
        if (Array.isArray(allProblems)) {
          availableToAdd = allProblems
            ...
            .map(problem => ({
              ...problem,
              title: problem.title ? decodeFromBase64(problem.title) : 'Untitled Problem'
            }));
        }
  ...
  const addProblemToContest = async (problemId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:3000/api/contests/addProblemToContest', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to add problem: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <div className='flex flex-col items-center min-h-screen bg-gray-900 py-10 px-4'>
      ...
      <div className="mb-6 w-full max-w-2xl flex justify-between">
        <button
          ...
          className={`${showAddProblem ? 'bg-gray-600 hover:bg-gray-700' : 'bg-amber-500 hover:bg-amber-600'} text-white px-6 py-3 rounded-lg transition font-semibold`}
        >
      ...
      <div
        className={`w-full max-w-2xl transition-all duration-500 ease-in-out overflow-hidden ${
          showAddProblem ? 'max-h-[500px] opacity-100 mb-8' : 'max-h-0 opacity-0'
        }`}
      >
        {showAddProblem && (
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 shadow-xl">
            <h3 className="text-white text-xl font-semibold mb-4">Add Problem to Contest</h3>
                ...
                filteredProblems.map((problem) => (
                  ...
                  >
                    <div className="flex-1 pr-4">
                      <h4 className="text-white font-medium text-lg">{problem.title}</h4>
      ...
      {problems.length === 0 ? (
        <div className="w-full max-w-2xl bg-gray-800 rounded-lg p-8 text-center">
          <p className="text-lg text-gray-400">No problems have been added to this contest yet.</p>
          <p className="text-gray-500 mt-2">Use the "Add Problems" button to add problems.</p>
        </div>
      ) : (
